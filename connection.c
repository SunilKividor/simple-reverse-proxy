...
void connection_on_out_event(struct epoll_event_handler* self) {
    ...
    while(closure->write_buffer != NULL) {
        ...
        if(written != to_write) {
            if(written == -1) {
                if(errno == ECONNRESET || errno == EPIPE) {
                    printf("On out event write error");
                    connection_on_close_event(self);
                    return;
                }
                if(errno != EAGAIN && errno != EWOULDBLOCK) {
...
struct epoll_event_handler* create_connection(int socket_fd) {
    ...
    printf("Created connection epoll handler %p\n",result);
    result->fd = socket_fd;
